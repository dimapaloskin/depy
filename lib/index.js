...
function Depy(directory, options) {
  ...
  this.projectCacheDir = path.resolve(path.join(this.options.cacheDir, '.cache', cacheEncodedName));
}
...
Depy.prototype.run = function () {
  ...
  if (files['yarn.lock'] && files['yarn.lock'].exists) {
  ...
  config.files.forEach(file => {
    try {
      ...
      if (stat.isFile()) {
        ...
        files[file].absolute = absolutePath;
